...
const state = { tasks: [], filter: 'all', blocksByTask: {}, blockingTaskId: null };
...
function taskCard(task) {
  ...
  const activeBlock = state.blocksByTask[task.id];
  return `<article class="task-card ${done ? 'done' : ''} ${task.is_blocked ? 'blocked' : ''}">
    ...
    <div class="task-main"><div class="task-title-row"><h3>${escapeHtml(task.title)}</h3><span class="status-pill ${task.is_blocked ? 'blocked' : task.status}">${statusText(task)}</span></div>
      ...
      <div class="task-meta"><span>${formatEstimatedHours(task.estimated_active_minutes)}</span><span>更新于 ${formatDate(task.updated_at)}</span>${task.is_blocked ? `<span class="block-meta">${escapeHtml(activeBlock?.reason || '需要处理依赖')}</span>` : ''}</div>
      ${task.is_blocked && activeBlock ? `<div class="block-actions"><button class="secondary-button small" data-action="unblock" data-block-id="${activeBlock.id}" data-block-version="${activeBlock.version}" type="button">解除阻塞</button></div>` : ''}
    </div>
    <div class="task-actions">${!done && !task.is_blocked ? `<button class="icon-button" data-action="work" data-id="${task.id}" title="开始工作" type="button">▶</button>` : ''}${!done && !task.is_blocked ? `<button class="icon-button" data-action="block" data-id="${task.id}" title="标记阻塞" type="button">⏸</button>` : ''}<button class="icon-button danger" data-action="delete" data-id="${task.id}" data-version="${task.version}" title="删除" type="button">⌫</button></div>
  </article>`;
}
...
function render() {
  ...
  app.innerHTML = `<main class="shell">
    ...
    <dialog id="block-dialog"><form method="dialog" id="block-form" class="dialog-card"><button class="dialog-close" value="cancel" aria-label="关闭">×</button><span class="eyebrow">BLOCK TASK</span><h2>标记阻塞</h2><p class="block-task-hint" id="block-task-title"></p><label>阻塞原因<textarea name="reason" rows="2" required maxlength="300" placeholder="例如：等待接口文档" autofocus></textarea></label><label>补充备注<textarea name="note" rows="2" placeholder="可选"></textarea></label><div class="dialog-actions"><button class="secondary-button" value="cancel">取消</button><button class="primary-button" value="default">确认阻塞</button></div></form></dialog>`;
