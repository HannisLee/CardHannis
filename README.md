...
- 桌面端会按配置自动双向合并 Supabase 数据，默认每 10 分钟一次；Web 设置页提供「立即同步」按钮，左下角显示数据库连接状态。首次同步若本地只有迁移产生的默认记录且远端已有数据，会直接采用远端快照，避免把本地种子上传。同步使用 Supabase Auth 登录用户和按 `user_id` 隔离的 RLS。
