...
impl TaskStore {
    ...
    pub fn create_task_at(&self, input: NewTask, created_at: impl Into<String>) -> Result<Task> {
        ...
        connection.execute(
            "INSERT INTO tasks (id, title, notes, estimated_active_minutes, created_at, sort_order, created_device_id, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?5)",
            params![id, input.title.trim(), input.notes, input.estimated_active_minutes, created_at, input.sort_order, input.created_device_id.trim()],
        )?;
        drop(connection);
    ...
    pub fn update_task(
        ...
        let changed = connection.execute(
            "UPDATE tasks SET title = ?1, notes = ?2, review_notes = ?3, estimated_active_minutes = ?4, sort_order = ?5, updated_at = ?6, version = version + 1 WHERE id = ?7 AND version = ?8 AND deleted_at IS NULL",
            params![title.trim(), notes, review_notes, estimated_active_minutes, sort_order, updated_at.into(), id, expected_version],
        )?;
        if changed == 0 {
    ...
    pub fn end_block(
        ...
        let ended_at = ended_at.into();
        let connection = self.connection.lock().expect("database mutex poisoned");
        let changed = connection.execute("UPDATE task_blocks SET ended_at = ?1, updated_at = ?1, version = version + 1 WHERE id = ?2 AND version = ?3 AND ended_at IS NULL AND deleted_at IS NULL", params![ended_at, block_id, expected_version])?;
        if changed == 0 {
            return Err(resolve_block_update_error(
                &connection,
                block_id,
                expected_version,
            ));
...
const TASK_BY_ID_SQL: &str = "SELECT t.id, t.title, t.notes, t.review_notes, t.estimated_active_minutes, t.created_at, t.started_at, t.completed_at, t.status, t.sort_order, t.created_device_id, t.updated_at, t.deleted_at, t.version, EXISTS (SELECT 1 FROM task_blocks b WHERE b.task_id = t.id AND b.ended_at IS NULL AND b.deleted_at IS NULL) FROM tasks t WHERE t.id = ?1";
const TASK_SELECT_SQL: &str = "SELECT t.id, t.title, t.notes, t.review_notes, t.estimated_active_minutes, t.created_at, t.started_at, t.completed_at, t.status, t.sort_order, t.created_device_id, t.updated_at, t.deleted_at, t.version, EXISTS (SELECT 1 FROM task_blocks b WHERE b.task_id = t.id AND b.ended_at IS NULL AND b.deleted_at IS NULL) FROM tasks t ORDER BY t.sort_order, t.created_at";
